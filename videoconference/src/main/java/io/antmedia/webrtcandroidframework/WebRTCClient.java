...
public class WebRTCClient implements IWebRTCClient, AntMediaSignallingEvents, IDataChannelMessageSender {
    ...
    final int RECONNECTION_PERIOD_MLS = 2000;
        ...
        reconnectionRunnable = new Runnable() {
            ...
            public void run() {
                if (!streamStoppedByUser) {
                    ...
                    Log.d(TAG, "run:  size of peer info ->"+peers.size());
                    for (PeerInfo peerInfo : peers.values()) {
                        PeerConnection pc = peerInfo.peerConnection;
                        Log.d(TAG, "run: createReconnectionRunnable -> peerInfo mode"+peerInfo.mode);
                        Log.d(TAG, "run: createReconnectionRunnable -> for "+peerInfo.streamName);
                        if(pc ==null) Log.d(TAG, "run: createReconnectionRunnable pc is null");
                        if(pc!=null)
                        {
                            Log.d(TAG, "run: pc iceConnectionState -> "+pc.iceConnectionState().toString());
                        }
                        if (pc == null ||
                                (pc != null
                            ...
                            Log.d(TAG, "run: createReconnectionRunnable inside if for checking the peerConnection ice connecion state");
                            noNeedToRetry = false;

                            if (pc != null) {
                                pc.dispose();
                            }
//                            disconnectWithErrorMessage("for reconnection -> ");
                            Log.d(TAG, "run: createReconnectionRunnable ");
                            ...
                            Log.d(TAG, "run: reconnectionRunnable peer conection is null");
                        }
                    }
    ...
    class TrackCheckTask extends TimerTask {
        ...
        public void run() {
            ...
            /*custom change Naghma 17 Oct 2023*/
            try {
                ...
                Log.e(TAG, "run:  TrackCheckTask  exception caught!!!");
            }
        }
    ...
    void onAudioManagerDevicesChanged(
        ...
        /*custom change -> naghma 27 Sep 2023*/
        Intent audioDeviceUpdateBroadcast = new Intent("sdk_broadcast_custom");
        audioDeviceUpdateBroadcast.putExtra("task", "audio_device_update");
        context.sendBroadcast(audioDeviceUpdateBroadcast);
    }
...
//        Log.d(TAG, "onCameraError: CAM_TEST onCamera Error 0> Error desc"+errorDescription.toString());
//        webRTCListener.onCameraError(errorDescription);
//    }
//
//    @Override
//    public void onCameraDisconnected() {
//        Log.d(TAG, "onCameraDisconnected:CAM_TEST ");
//
//    }
//
//    @Override
//    public void onCameraFreezed(String errorDescription) {
//        Log.d(TAG, "onCameraFreezed: CAM_TEST error desc -> "+errorDescription);
//    }
//
//    @Override
//    public void onCameraOpening(String cameraName) {
//        Log.d(TAG, "onCameraOpening: CAM_TEST onCamera opening -> camera name "+cameraName);
//    }
//
//    @Override
//    public void onFirstFrameAvailable() {
//        Log.d(TAG, "onFirstFrameAvailable: CAM_TEST on First frame available -> ");
//    }
//
//    @Override
//    public void onCameraClosed() {
//        Log.d(TAG, "onCameraClosed: CAM_TEST onCamera closed");
//    }

    /*custom 22 Oct 2023 to get status of the permission*/
    public boolean getIsScreenSharePermissionRequired(){
